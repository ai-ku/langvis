	 ...
	 */
	 ...
	 throws IOException {
		 ...
		 case bKGDChunk:
			 ...
			 case COLOR | ALPHA:
				 pngassert(len == 6);
				 c = new Color(buf[st] & 0xff, buf[st + 2] & 0xff,
						 buf[st + 4] & 0xff);
				 break;
			 case COLOR | PALETTE:
			 case COLOR | PALETTE | ALPHA:
				 ...
				 c = new Color(red_map[ix] & 0xff, green_map[ix] & 0xff,
						 blue_map[ix] & 0xff);
				 break;
			 case GRAY:
			 case GRAY | ALPHA:
		 ...
		 case cHRMChunk:
			 property("chromaticities", new Chromaticities(getInt(st),
					 getInt(st + 4), getInt(st + 8), getInt(st + 12),
					 getInt(st + 16), getInt(st + 20), getInt(st + 24),
					 getInt(st + 28)));
			 break;
		 case gAMAChunk:
			 if (len != 4)
		 ...
		 case tEXtChunk:
			 ...
			 if (klen < len) {
				 ...
				 String tvalue = new String(buf, st + klen + 1, len
						 - klen - 1);
		 ...
		 case tIMEChunk:
			 property("modtime", new GregorianCalendar(getShort(st + 0),
					 getByte(st + 2) - 1, getByte(st + 3),
					 getByte(st + 4), getByte(st + 5), getByte(st + 6))
			 .getTime());
			 break;
		 case tRNSChunk:
			 switch (colorType) {
			 ...
			 case PALETTE | COLOR | ALPHA:
				 ...
				 System.arraycopy(buf, st, alpha_map, 0,
						 len < alen ? len : alen);
				 while (--alen >= len)
			 ...
			 case COLOR | ALPHA: // doesn't deal with 16 bit colors properly
				 ...
				 if (bitDepth == 16) {
					 ...
					 transparentPixel = ((getShort(st + 0) & 0xFF) << 16)
	 ...
	 public void produceImage() throws IOException, ImageFormatException {
		 ...
		 try {
			 for (int i = 0; i < signature.length; i++)
				 if ((signature[i] & 0xFF) != underlyingInputStream
						 .read())
					 throw new PNGException("Chunk signature mismatch");

			 InputStream is = new BufferedInputStream(
					 new InflaterInputStream(inputStream, new Inflater()));
			 ...
			 case GRAY: {
				 ...
				 if ((cm = greyModels[llog]) == null) {
					 ...
					 if (transparentPixel == -1) {
						 cm = new IndexColorModel(bitDepth, ramp.length,
								 ramp, ramp, ramp);
					 } else {
						 cm = new IndexColorModel(bitDepth, ramp.length,
								 ramp, ramp, ramp,
			 ...
			 int samplesPerPixel = ((colorType & PALETTE) != 0 ? 1
					 : ((colorType & COLOR) != 0 ? 3 : 1)
					 + ((colorType & ALPHA) != 0 ? 1 : 0));
			 ...
			 int bytesPerPixel = (bitsPerPixel + 7) >> 3;
                    ...
                    while(++pass<=passLimit && (t=target)!=null) {
					 ...
					 while (++pass <= passLimit) {
						 ...
						 int rowPixelWidth = (width - sCol + (colInc - 1))
						 / colInc;
						 int rowByteWidth = ((rowPixelWidth * bitsPerPixel) + 7) >> 3;
					 if (rowByteWidth == 0)
						 continue;
					 int pixelBufferInc = interlaceMethod == 0 ? rowInc
							 * width : 0;
					  ...
					  */
					  while (row < height) {
						  ...
						  for (int rowFillPos = 0; rowFillPos < rowByteWidth;) {
							  int n = is.read(rowByteBuffer, rowFillPos,
									  rowByteWidth - rowFillPos);
							  if (n <= 0)
								  throw new PNGException("missing data");
							  rowFillPos += n;
						  }
						  filterRow(rowByteBuffer, firstRow ? null
								  : prevRowByteBuffer, rowFilter,
								  rowByteWidth, bytesPerPixel);
						  int col = sCol;
						  int spos = 0;
						  int pixel = 0;
						  while (col < width) {
							  if (wPixels != null) {
								  ...
								  for (int i = 0; isTransparent
								  && (i < 6); i++) {
									  isTransparent &= (rowByteBuffer[spos
									                                  + i] & 0xFF) == (transparentPixel_16[i] & 0xFF);
								  }
								  if (!isTransparent) {
									  pixel |= 0xff000000;
								  }
						  ...
						  if (interlaceMethod == 0)
							  if (wPixels != null) {
								   ...
								   */
								   ...
								   sendPixels(0, row, width, 1, wPixels, 0,
										   width);
								   // end of adding
							  } else {
								  /* code changed. target not needed here
								   ...
								   */
								   // code added to make it work with ImageDecoder arch
								   sendPixels(0, row, width, 1, bPixels, 0,
										   width);
								   //end of adding
							  }
						  row += rowInc;
					  ...
					  if (interlaceMethod != 0)
						  if (wPixels != null) {
							   ...
							   */
							  // code added to make it work with ImageDecoder arch
							  sendPixels(0, 0, width, height, wPixels, 0,
									  width);
							  //end of adding
						  } else {
							  /* code changed. target not needed here
							   ...
							   */
							  // code added to make it work with ImageDecoder arch
							  sendPixels(0, 0, width, height, bPixels, 0,
									  width);
							  //end of adding
						  }
					 }
	 ...
	 private boolean sendPixels(int x, int y, int w, int h,
			 int[] pixels, int offset, int pixlength) {
	 ...
	 private boolean sendPixels(int x, int y, int w, int h,
			 byte[] pixels, int offset, int pixlength) {
		 int count = setPixels(x, y, w, h, cm, pixels, offset, pixlength);
		 if (count <= 0) {
			 aborted = true;
		 }
	 ...
	 throws IOException {
		 ...
		 case 3:
			 if (prevRow != null) {
				 ...
				 for (; x < rowByteWidth; x++)
					 rowByteBuffer[x] += ((prevRow[x] & 0xFF) + (rowByteBuffer[x
					                                                           - bytesPerSample] & 0xFF)) >> 1;
			 } else
				 for (x = bytesPerSample; x < rowByteWidth; x++)
					 rowByteBuffer[x] += (rowByteBuffer[x
					                                    - bytesPerSample] & 0xFF) >> 1;
				 break;
				 case 4:
					 if (prevRow != null) {
	 ...
	 //abstract public class ChunkReader extends FilterInputStream {
		 ...
		 private static final byte[] signature = { (byte) 137, (byte) 80,
			 (byte) 78, (byte) 71, (byte) 13, (byte) 10, (byte) 26,
			 (byte) 10 };
		 ...
		 private void fill() throws IOException {
			 if (!seenEOF) {
				 ...
				 while (limit < bsize) {
					 int n = underlyingInputStream.read(inbuf, limit, bsize
							 - limit);
					 if (n <= 0) {
						 seenEOF = true;
						 break;
		 ...
		 static private int update_crc(int crc, byte[] buf, int offset,
				 int len) {
			 int c = crc;
			 while (--len >= 0)
		 ...
		 public static class Chromaticities {
			 public float whiteX, whiteY, redX, redY, greenX, greenY, blueX,
			 blueY;

			 Chromaticities(int wx, int wy, int rx, int ry, int gx, int gy,
					 int bx, int by) {
				 whiteX = wx / 100000.0f;
				 whiteY = wy / 100000.0f;
				 redX = rx / 100000.0f;
			 ...
			 public String toString() {
				 return "Chromaticities(white=" + whiteX + "," + whiteY
						 + ";red=" + redX + "," + redY + ";green=" + greenX
