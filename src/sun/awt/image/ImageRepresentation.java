...
public class ImageRepresentation extends ImageWatched implements ImageConsumer {
	 ...
	 * be used.
	 */
	public ImageRepresentation(ToolkitImage im, ColorModel cmodel,
			boolean forceCMhint) {
		image = im;
	...
	public void setDimensions(int w, int h) {
		...
		newInfo(image, (ImageObserver.WIDTH | ImageObserver.HEIGHT), 0,
				0, w, h);
	...
	void createBufferedImage() {
		...
		isDefaultBI = false;
		try {
			biRaster = cmodel.createCompatibleWritableRaster(width,
					height);
			bimage = createImage(cmodel, biRaster, cmodel
	...
	private void convertToRGB() {
		...
		if (cmodel instanceof  IndexColorModel
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					srcpixels = biRaster.getDataElements(x, y,
							srcpixels);
		...
		biRaster = Raster.createPackedRaster(dbi, w, h, w, bandMasks,
				null);
	...
	public native void setICMpixels(int x, int y, int w, int h,
			int[] lut, byte[] pix, int off, int scansize,
			IntegerComponentRaster ict);

	public native void setBytePixels(int x, int y, int w, int h,
			byte[] pix, int off, int scansize, ByteComponentRaster bct,
			int chanOff);

	public native int setDiffICM(int x, int y, int w, int h, int[] lut,
			int transPix, int numLut, IndexColorModel icm, byte[] pix,
			int off, int scansize, ByteComponentRaster bct, int chanOff);
				...
				synchronized (this ) {
					...
					if (isSameCM && (cmodel != model) && (srcLUT != null)
						...
						if (setDiffICM(x, y, w, h, srcLUT, srcLUTtransIndex,
								numSrcLUT, icm, pix, off, scansize, bct, bct
								.getDataOffset(0)) == 0) {
							convertToRGB();
						} else {
							...
							if (numlut != numSrcLUT) {
								...
								icm = new IndexColorModel(nbits, numSrcLUT,
										srcLUT, 0, hasAlpha, srcLUTtransIndex,
										(nbits > 8 ? DataBuffer.TYPE_USHORT
					...
					if (isDefaultBI) {
						...
						if (srcLUT != null && model instanceof  IndexColorModel) {
							...
							if (s_useNative) {
								...
								setICMpixels(x, y, w, h, srcLUT, pix, off,
										scansize, iraster);
								iraster.markDirty();
							} else {
				...
				if (isDefaultBI) {
					...
					if (cmodel.equals(model)) {
						...
						for (yoff = 0; yoff < h; yoff++, lineOff += scansize) {
							System.arraycopy(pix, lineOff, data, doff,
									w);
							doff += sstride;
						}
						...
						for (yoff = y; yoff < y + h; yoff++, lineOff += scansize) {
							...
							iraster.setDataElements(x, yoff, w, 1,
									storage);
						}
					}
	...
	public BufferedImage getOpaqueRGBImage() {
		if (bimage.getType() == BufferedImage.TYPE_INT_ARGB) {
			...
			ColorModel opModel = new DirectColorModel(24, 0x00ff0000,
					0x0000ff00, 0x000000ff);

			int bandmasks[] = { 0x00ff0000, 0x0000ff00, 0x000000ff };
			WritableRaster opRaster = Raster.createPackedRaster(db, w,
					h, w, bandmasks, null);
	...
	public boolean prepare(ImageObserver iw) {
		...
		if ((availinfo & ImageObserver.ERROR) != 0) {
			if (iw != null) {
				iw.imageUpdate(image, ImageObserver.ERROR
	...
	public boolean drawToBufImage(Graphics g, ToolkitImage img, int x,
			int y, Color bg, ImageObserver iw) {
	...
	public boolean drawToBufImage(Graphics g, ToolkitImage img, int x,
			int y, int w, int h, Color bg, ImageObserver iw) {
	...
	public boolean drawToBufImage(Graphics g, ToolkitImage img,
		...
		if (done || (0 != (availinfo & ImageObserver.FRAMEBITS))) {
			g.drawImage(bimage, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2,
					bg, null);
		}
	...
	public boolean drawToBufImage(Graphics g, ToolkitImage img,
			AffineTransform xform, ImageObserver iw) {
	...
	synchronized void abort() {
		...
		availinfo &= ~(ImageObserver.SOMEBITS | ImageObserver.FRAMEBITS
				| ImageObserver.ALLBITS | ImageObserver.ERROR);
	}
