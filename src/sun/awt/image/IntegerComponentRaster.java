...
public class IntegerComponentRaster extends SunWritableRaster {
	...
	public IntegerComponentRaster(SampleModel sampleModel,
		...
		if (dbi.getNumBanks() != 1) {
			throw new RasterFormatException(
					"DataBuffer for IntegerComponentRasters"
			...
			* scanlineStride + (dstX - minX);
			...
			if (ict.getPixelStride() == pixelStride) {
				...
				for (int startY = 0; startY < height; startY++) {
					System.arraycopy(tdata, srcOffset, data, dstOffset,
							width);
					srcOffset += tss;
	...
	public WritableRaster createWritableChild(int x, int y, int width,
		...
		return new IntegerComponentRaster(sm, dataBuffer,
				new Rectangle(x0, y0, width, height), new Point(
						sampleModelTranslateX + deltaX,
	...
	public Raster createChild(int x, int y, int width, int height,
		...
		return createWritableChild(x, y, width, height, x0, y0,
				bandList);
	}
	...
	private void verify(boolean strictCheck) {
		if (dataOffsets[0] < 0) {
			throw new RasterFormatException("Data offset ("
					+ dataOffsets[0] + ") must be >= 0");
		}
	...
	public String toString() {
		return new String("IntegerComponentRaster: width = " + width
				+ " height = " + height + " #Bands = " + numBands
				+ " #DataElements " + numDataElements + " xOff = "
				+ sampleModelTranslateX + " yOff = "
				+ sampleModelTranslateY + " dataOffset[0] "
				+ dataOffsets[0]);
	}
