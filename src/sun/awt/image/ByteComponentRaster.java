...
public class ByteComponentRaster extends SunWritableRaster {
	...
	public ByteComponentRaster(SampleModel sampleModel, Point origin) {
		this (sampleModel, sampleModel.createDataBuffer(),
				new Rectangle(origin.x, origin.y, sampleModel
						.getWidth(), sampleModel.getHeight()), origin,
						null);
	}
	...
	public ByteComponentRaster(SampleModel sampleModel,
		...
		if (dbb.getNumBanks() != 1) {
			throw new RasterFormatException(
					"DataBuffer for ByteComponentRasters"
		...
		if (sampleModel instanceof  ComponentSampleModel) {
			...
			for (int i = 0; i < getNumDataElements(); i++) {
				dataOffsets[i] += dbOffset + xOffset * pixelStride
			...
			int xOffset = aRegion.x - origin.x;
			int yOffset = aRegion.y - origin.y;
			dataOffsets[0] += xOffset * pixelStride + yOffset
			* scanlineStride;
		} else {
			throw new RasterFormatException(
					"IntegerComponentRasters must "
	 ...
	 * @param inRaster   Raster of data to place at x,y location.
	 */
	private void setDataElements(int dstX, int dstY, int width,
			int height, Raster inRaster) {
		...
		if (inRaster instanceof  ByteComponentRaster) {
			...
			if (numDataElements == 1) {
				...
				int srcOffset = toff;
				int dstOffset = dataOffsets[0] + (dstY - minY)
				* scanlineStride + (dstX - minX);

				if (pixelStride == bct.getPixelStride()) {
					width *= pixelStride;
					for (int tmpY = 0; tmpY < height; tmpY++) {
						System.arraycopy(bdata, srcOffset, data,
								dstOffset, width);
						srcOffset += tss;
	 ...
	 */
	public void putByteData(int x, int y, int w, int h, int band,
			byte[] inData) {
		// Bounds check for 'band' will be performed automatically
		if ((x < this .minX) || (y < this .minY) || (x + w > this .maxX)
				|| (y + h > this .maxY)) {
	 ...
	 */
	public Raster createChild(int x, int y, int width, int height,
			int x0, int y0, int[] bandList) {
		WritableRaster newRaster = createWritableChild(x, y, width,
				height, x0, y0, bandList);
	...
	public WritableRaster createWritableChild(int x, int y, int width,
		...
		return new ByteComponentRaster(sm, dataBuffer, new Rectangle(
				x0, y0, width, height), new Point(sampleModelTranslateX
						+ deltaX, sampleModelTranslateY + deltaY), this );
	}
	...
	private void verify(boolean strictCheck) {
		...
		for (int i = 0; i < dataOffsets.length; i++) {
			if (dataOffsets[i] < 0) {
				throw new RasterFormatException(
						"Data offsets for band " + i + "("
						+ dataOffsets[i] + ") must be >= 0");
			}
		}
		...
		for (int i = 0; i < numDataElements; i++) {
			size = (height - 1) * scanlineStride + (width - 1)
			* pixelStride + dataOffsets[i];
			if (size > maxSize) {
				maxSize = size;
			}
		}
		if (data.length < maxSize) {
			throw new RasterFormatException(
					"Data array too small (should be " + maxSize + " )");
		}
	}

	public String toString() {
		return new String("ByteComponentRaster: width = " + width
