	 ...
	 */
			 ...
			 while (!aborted) {
					 ...
					 default:
						 ...
						 while (true) {
							 ...
							 if (code == EX_COMMENT) {
								 ...
								 if ("NETSCAPE2.0".equals(new String(
										 buf, 0))) {
									 loop_tag = true;
								 }
							 }
				 ...
				 case -1:
					 if (verbose) {
						 if (code == -1) {
							 System.err
							 .println("Premature EOF in GIF file,"
									 + " frame " + frameno);
						 } else {
							 System.err
							 .println("corrupt GIF file (parse) ["
									 + code + "].");
						 }
					 }
					 if (frameno == 0) {
	 ...
	 private void readHeader() throws IOException, ImageFormatException {
		 ...
		 if ((ch & COLORMAPMASK) == 0) {
			 ...
			 if (buf[12] != 0) {
				 props.put("aspectratio", ""
						 + ((ExtractByte(buf, 12) + 15) / 64.0));
			 }
	 ...
	 private native boolean parseImage(int x, int y, int width,
			 int height, boolean interlace, int initCodeSize,
	 ...
	 private int sendPixels(int x, int y, int width, int height,
		 ...
		 if (trans_pixel >= 0 && !curframe.initialframe) {
			 if (saved_image != null && model.equals(saved_model)) {
				 ...
				 for (int i = rasbeg; i < rasend; i++, off++) {
					 ...
					 if ((pixel & 0xff) == trans_pixel) {
						 if (runstart >= 0) {
							 count = setPixels(x + runstart, y, i
									 - runstart, 1, model, rasline,
									 runstart, 0);
							 if (count == 0) {
								 break;
							 }
				 ...
				 if (runstart >= 0) {
					 count = setPixels(x + runstart, y, rasend
							 - runstart, 1, model, rasline, runstart, 0);
				 }
				 return count;
			 }
		 } else if (save) {
		 ...
		 int count = setPixels(x2, y, width, height, model, rasline,
				 rasbeg, 0);
		 return count;
	 }
	  ...
	  * Read Image data
	  */
	 private boolean readImage(boolean first, int disposal_method,
		 ...
		 if ((block[8] & COLORMAPMASK) != 0) {
			 ...
			 model = new IndexColorModel(8, num_local_colors,
					 local_colormap, 0, false, trans_pixel);
		 } else if (model == null
				 || trans_pixel != model.getTransparentPixel()) {
			 if (trans_pixel >= num_global_colors) {
				 // Fix for 4233748: extend colormap to contain transparent pixel
				 num_global_colors = trans_pixel + 1;
				 global_colormap = grow_colormap(global_colormap,
						 num_global_colors);
			 }
			 model = new IndexColorModel(8, num_global_colors,
					 global_colormap, 0, false, trans_pixel);
			 global_model = model;
		 }
		 ...
		 if (disposal_method == GifFrame.DISPOSAL_SAVE
				 && saved_image == null) {
			 ...
			 if ((height < global_height) && (model != null)) {
				 ...
				 if (tpix >= 0) {
					 ...
					 setPixels(0, 0, global_width, y, model,
							 trans_rasline, 0, 0);
					 setPixels(0, y + height, global_width,
							 global_height - height - y, model,
							 trans_rasline, 0, 0);
				 }
			 }
		 }
		 ...
		 curframe = new GifFrame(this , disposal_method, delay,
				 (curframe == null), model, x, y, width, height);
		 ...
		 boolean ret = parseImage(x, y, width, height, interlace,
				 ExtractByte(block, 9), block, rasline, model);
...
class GifFrame {
	...
	private void setPixels(int x, int y, int w, int h, ColorModel cm,
	...
	public boolean dispose() {
		if (decoder.imageComplete(ImageConsumer.SINGLEFRAMEDONE, false) == 0) {
			...
			if (verbose && disposal_method != 0) {
				System.out.println("disposal method: "
						+ disposal_method);
			}
			...
			case DISPOSAL_PREVIOUS:
				...
				if (saved_image != null) {
					setPixels(x, y, width, height, saved_model,
							saved_image, y * global_width + x,
							global_width);
				}
				break;
			case DISPOSAL_BGCOLOR:
				...
				if (model.getTransparentPixel() < 0) {
					model = trans_model;
					if (model == null) {
						model = new IndexColorModel(8, 1, new byte[4],
								0, true);
						trans_model = model;
					}
					tpix = 0;
