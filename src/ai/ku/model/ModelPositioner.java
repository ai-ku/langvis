...
public class ModelPositioner {

	private static ModelPositioner mp = null;

	// Singleton
	public static ModelPositioner getInstance(){
		if( mp == null )
			mp = new ModelPositioner();
		return mp;
	}
	
	private ModelPositioner(){
	...
	// Places AIModel A near the AIModel B
	// A is placed relative to B
	// Position of A will change after this method
	// Position of B should not change
	public void placeNear( AIModel a, AIModel b ) {
		...
		Vector3d aCenter = a.getCenter();
		Vector3d bCenter = b.getCenter();
		// double bGround = bpos.y - b.height()/2;
		// double aCenterY = bGround + a.height()/2;
		
		double bGround = bCenter.y - b.height()/2;
		double aCenterY = bGround + a.height()/2;
		
		/*
		System.out.println(bpos.y);
		System.out.println(b.height());
		
		System.out.println(a.height());
		System.out.println(apos.y);
		*/
		
		A.setPositionRightNow( new Vector3d( bpos.x, aCenterY, bpos.z ), A.getWorld() );
