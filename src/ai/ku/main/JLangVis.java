...
public class JLangVis {
	...
	private static String backgroundColor =  new edu.cmu.cs.stage3.alice.scenegraph.Color( 0.0/255.0, 0.0/255.0, 255.0/255.0 ).toString();
	static {
		try {
	...
	public static void main( String[] args ) {		
		...
		try {
			...
			/* modified by emre */ 
			boolean useJavaBasedSplashScreen = false; // true
	...
	private static void configInit() {
		final edu.cmu.cs.stage3.alice.authoringtool.util.Configuration authoringtoolConfig = edu.cmu.cs.stage3.alice.authoringtool.util.Configuration.getLocalConfiguration( AuthoringTool.class.getPackage() );
...
//		);
		...
		if( authoringtoolConfig.getValue( "mainWindowBounds" ) == null ) {
			...
			authoringtoolConfig.setValue( "mainWindowBounds", x + ", " + y + ", " + screenWidth + ", " + height );
		}
	...
	public static java.io.File getAliceUserDirectory() {
		if( aliceUserDirectory == null ) {
			...
			if( dirFromProperties != null ) {
				...
				aliceUser = new java.io.File( userHome, ".alice" );
			} else if( (aliceHome != null) && aliceHome.exists() && aliceHome.canRead() && aliceHome.canWrite() ) {
				aliceUser = new java.io.File( aliceHome, ".alice" );
			}
