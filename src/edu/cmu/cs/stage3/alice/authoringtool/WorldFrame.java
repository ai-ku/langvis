...
import ai.ku.component.AIButton;
import ai.ku.component.AITextArea;
import ai.ku.component.AITextField;
import ai.ku.model.ModelExtractor;
import ai.ku.util.Logger;
import ai.ku.util.Palette;
import ai.ku.util.Sleeper;
import cmu.component.ComponentImageFactory;
 ...
 * @author Jason Pratt
 */
public class WorldFrame extends javax.swing.JFrame implements ActionListener{
	...
	ComponentImageFactory componentImageFactory = new ComponentImageFactory();
	StatusBar statusBar;
	...
	public WorldFrame( AuthoringTool authoringTool ) {
	...
	private AITextField textfield;
	private AITextArea textarea;
	
	// private String sampleText = "A girl stood near the man in front of the car.";
	// private String sampleText = "A monkey stood in front of the car, behind the tree.";
	// private String sampleText = "A house is in front of the tree, behind the man.";
	// private String sampleText = "A crow is on the tree. A fox is behind the tree. The cheese is on the crow.";
	// private String sampleText = "A man stood in front of the house.";
	private String sampleText = "A penguin stood in front of the tree. A dog is behind the tree.";
	
	public void initAIUI(){
		
		extractor = new ModelExtractor( authoringTool, this );
		textfield = new AITextField();
		textfield.setText( sampleText );
		
		// textarea = new AITextArea();
		// textarea.setText( sampleText );
				
		JPanel inputPanel = new JPanel( new GridLayout( 1, 2 ) );
		JPanel buttonPanel = new JPanel( new GridLayout( 1, 2 ) );
		
		AIButton doButton = new AIButton( "Do your thing." );
		doButton.setActionCommand( "do" );
		doButton.addActionListener( this );
		doButton.setIcon( AuthoringToolResources.getIconForString( "magic" ) );
		// doButton.setAction(doAction);
		
		AIButton resetButton = new AIButton( "Reset" );
		resetButton.setActionCommand( "reset" );
		resetButton.setIcon( AuthoringToolResources.getIconForString( "broom" ) );
		resetButton.addActionListener( this );
		// resetButton.setAction(actions.newWorldAction);
		
		AIButton queryButton = new AIButton("Question me!");
		queryButton.setActionCommand("question");
		// queryButton.setIcon(null);
		queryButton.addActionListener(this);
		
		AIButton cameraButton = new AIButton("Set Camera");
		cameraButton.setActionCommand("camera");
		// queryButton.setIcon(null);
		cameraButton.addActionListener(this);
		
		AIButton initButton = new AIButton("Init World");
		initButton.setActionCommand("init");
		// queryButton.setIcon(null);
		initButton.addActionListener(this);
		
		AIButton addBoxButton = new AIButton("Add Box");
		addBoxButton.setActionCommand("add");
		addBoxButton.addActionListener(this);
		
		AIButton startButton = new AIButton("Start Sim");
		startButton.setActionCommand("simulate");
		// queryButton.setIcon(null);
		...
		// Added for path planning
		AIButton birdsEyeButton = new AIButton("Bird's Eye");
		birdsEyeButton.setActionCommand("topview");
		birdsEyeButton.addActionListener(this);
		// buttonPanel.add(birdsEyeButton);
		
		// Turned off by emre for what i do demo 
		// buttonPanel.add( initButton );
		// buttonPanel.add( addBoxButton );
		// buttonPanel.add( startButton );
		buttonPanel.setBackground( Palette.gray );
	...
	public void actionPerformed(ActionEvent event) {
		...
		if( action.equals("do") )
		{
			...
			for (int end = iterator.next(); end != BreakIterator.DONE; start = end, end = iterator.next()) {
				...
				Logger.log( sentence );
				Sleeper.sleep(2000);
				extractor.extractModels( sentence.toLowerCase() );
				Sleeper.sleep(2000);
			}
			*/
		    ...
		    for(CoreMap sentence: sentences) {
		    	Logger.log( "Sentence:" + sentence.toString() );
				Sleeper.sleep(2000);
				extractor.extractModels( sentence.toString().toLowerCase() );
				Sleeper.sleep(2000);
		    }
			
			/*
			String[] sentences = text.split("(?<=[a-z])\\.\\s+");
			for (String sentence : sentences) { Logger.log(sentence); }
			*/
		}
		else if( action.equals("reset") )
			extractor.deleteModels();
		else if( action.equals("question") ) {
			extractor.startQuery();
		}
		else if( action.equals("camera") ) {
			extractor.getAGoodLookAtScene();
		}
		else if( action.equals("init") ) {
			extractor.initWorld();
		}
		else if( action.equals("simulate") ) {
			extractor.startWorld();
		}
		else if( action.equals("add") ) {
			extractor.addBox();
		} 
		else if( action.equals("topview") ) {
			extractor.showBirdsEyeView();
		}
	...
	private void guiInit() {
		...
		{ fileMenu.remove( WorldFrame.this.openZippedWorldItem ); }

		// getContentPane().setBackground( edu.cmu.cs.stage3.alice.scenegraph.Color.valueOf( authoringToolConfig.getValue( "backgroundColor" ) ).createAWTColor() );
		getContentPane().setBackground( Palette.gray );
				...
				new edu.cmu.cs.stage3.alice.authoringtool.util.event.ConfigurationListener() {
					...
					public void changed( edu.cmu.cs.stage3.alice.authoringtool.util.event.ConfigurationEvent ev ) {
						if( ev.getKeyName().equals( "edu.cmu.cs.stage3.alice.authoringtool.backgroundColor" ) ) {
							WorldFrame.this.getContentPane().setBackground( edu.cmu.cs.stage3.alice.scenegraph.Color.valueOf( authoringToolConfig.getValue( "backgroundColor" ) ).createAWTColor() );
							...
							if( ev.getNewValue().equalsIgnoreCase( "true" ) ) {
								...
								if( ! fileMenu.isMenuComponent( WorldFrame.this.openZippedWorldItem ) ) {
									fileMenu.add( WorldFrame.this.openZippedWorldItem, 2 );
								}
							}
						}
					}
	...
	private void recentWorldsInit() {
		recentWorldsListener = new java.awt.event.ActionListener() {
			public void actionPerformed( java.awt.event.ActionEvent ev ) {
				...
				if( WorldFrame.this.authoringTool.loadWorld( item.getText(), true ) == Constants.FAILED ) {
					WorldFrame.this.recentWorlds.remove( item.getText() );
					WorldFrame.this.updateRecentWorlds();
				}
			}
		};
	...
	JButton undoButton = new JButton("Undo Button");
	JButton redoButton = new JButton("Redo Button");
	...
	JButton teachMeButton = new JButton("teach me");
	...
	private void jbInit2(){
		...
		border4 = BorderFactory.createLineBorder(Color.gray,1);
		border5 = BorderFactory.createMatteBorder(0, 1, 0, 0, Color.gray);
		...
		this.getContentPane().add(leftRightSplitPane, BorderLayout.CENTER);
		leftRightSplitPane.add( worldTreePanel, JSplitPane.LEFT );
		leftRightSplitPane.setDividerLocation(100);
		leftRightSplitPane.setDividerSize(2);
		
		// this.getContentPane().add(scenePanel, BorderLayout.CENTER);
		leftRightSplitPane.add( scenePanel, JSplitPane.RIGHT );
		//((BasicSplitPaneDivider)leftRightSplitPane.getComponent(1)).setBackground(Color.black);
		*/
		...
		*/
		...
		// this.getContentPane().add(mainPanel, BorderLayout.CENTER);
		// mainPanel.add(authoringPanel, BorderLayout.CENTER);
		// authoringPanel.add( scenePanel, BorderLayout.CENTER );
		
		/*
		this.getContentPane().add(scenePanel, BorderLayout.CENTER);
		
		Actions actions = new Actions(authoringTool, this);	
		javax.swing.AbstractAction doAction = new javax.swing.AbstractAction() {
			private static final long serialVersionUID = 1L;
			public void actionPerformed( java.awt.event.ActionEvent e ) {}
		};
		
		doAction.putValue( javax.swing.Action.ACCELERATOR_KEY, javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_D, java.awt.Event.CTRL_MASK) );
		doAction.putValue( javax.swing.Action.ACTION_COMMAND_KEY, "doAction" );
		doAction.putValue( javax.swing.Action.MNEMONIC_KEY, new Integer( 'D' ) );
		doAction.putValue( javax.swing.Action.NAME, "Do your thing." );
		doAction.putValue( javax.swing.Action.SHORT_DESCRIPTION, "Do magic." );
		doAction.putValue( javax.swing.Action.SMALL_ICON, AuthoringToolResources.getIconForString( "magic" ) );
	...
	private void jbInit() {
		...
		leftRightSplitPane.add(scenePanel, JSplitPane.RIGHT); // rightPanel
